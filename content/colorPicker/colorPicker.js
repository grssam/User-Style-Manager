...
*/
function colorPicker(e,mode,size,rO/*readOnly*/,offsetX,offsetY,orientation,parentObj,parentXY,color,difPad,rSpeed,docBody){
  ...
  CP=(!cP)?function(obj) { // building the colorPicker / setting up events and methodes
    ...
    var innerDOM = scriptableUnescapeHTML.parseFragment(HTMLTxt.replace(/src=\"/g,'src="'+cPDir), false, null, div);
    ...
    scripts=div=n=m=cPDir=nN=nB=CSSTxt=HTMLTxt=_h=_blank=_horizontal=_vertical=_icons=_patches=_IE=_nonIE=testPix=filter=null;
  ...
  getOrigin = function(obj) {
    ...
    if (obj.getBoundingClientRect) { // IE
      ...
      if (window.opera  || (document.childNodes && !document.all && !navigator.taintEnabled && !accentColorName)) pos[1] -= docBody.offsetTop;
    }
  ...
  (function() {
    var obj = parentObj || e.target || e.srcElement, n, c;
